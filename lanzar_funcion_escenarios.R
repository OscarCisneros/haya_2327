...
grupo_ <- "go_fagus" #"go_fagus" #"planes_comarcales_navarra" "selv_macizo_pirenaico" "CNPF" "PC" "opt_go_fagus" "madrigal"
# densidades_iniciales_ <- c(4029,5469,7510,10396,14418)
# is_ <- c("IS25","IS22","IS19","IS16","IS13")
...
#-------------------------------------------------------------------------------
#lanzar escenarios con volumen minimo extraido de 40 m3. Selección de densidades iniciales. Relación de Dg y V de Madrigal et al., 1992
source("scripts/funcion_genera_escenarios_volumen_minimo_Madrigal.R")

grupo_ <-  "PC_madrigal" #"go_fagus" #"planes_comarcales_navarra" "selv_macizo_pirenaico" "CNPF" "label_bas_carbon" "mortalidad_natural" "prueba"
#densidades iniciales y calidades correspondientes, para retener sólo los pares admitidos
# densidades_iniciales_ <- c(4029,5469,7510,10396,14418)
# is_ <- c("IS25","IS22","IS19","IS16","IS13")

densidades_iniciales_ <- c(8000)
is_ <- c("IS25")
...
oo <- map_dfr(lista_$lista_var, ~funcion_genera_escenarios_vol_min_Madrigal(grupo_ = .[1], N_ini_ = .[2],escenario.nombre_ = .[3]) )
...
#Edades en función del tipo de masa en Aralar A,B,C,D corresponde con edades 50,30,100,70
#Densidades iniciales propuestas por Mikel
source("scripts/funcion_genera_escenarios_volumen_minimo_IRREG_madrigal.R")
source("scripts/funcion_genera_escenarios_transformacion_unido_desde_REGULAR_par_CO2Fix_.R")
source("scripts/funciones_accesorias_transformacion.R")

grupo_ <- "PC_madrigal" #"go_fagus" #"planes_comarcales_navarra" "selv_macizo_pirenaico" "CNPF" "PC" "opt_go_fagus" "madrigal"
# densidades_iniciales_ <- c(4029,5469,7510,10396,14418)
# is_ <- c("IS25","IS22","IS19","IS16","IS13")
...
oo <- map_dfr(lista_$lista_var, ~funcion_genera_escenarios_vol_min_IRREG_madrigal(grupo_ = .[1], N_ini_ = .[2], escenario.nombre_ = .[3]))
