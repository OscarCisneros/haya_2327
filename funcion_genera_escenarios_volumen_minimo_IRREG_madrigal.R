...
funcion_genera_escenarios_vol_min_IRREG_madrigal <- function(grupo_ = "prueba", escenario.nombre_ = "Prueba_H5_IS19_alta",
                                      N_ini_ = 7510) {
...
#comprobar si existe directorio para el escenario y en caso contrario crearlo
  ...
  if(!file.exists(ruta_dir_escenario)) {
    ...
    #Ecuaciones Madrigal
    # parámetros de la relación Dg ~ f(N, Ho)
    g0 = -8.74
    g1 = 3.6179
    g2 = 0.45
    g3 = 0.67544
    
    # parámetros de la relación V ~ f(G, Ho)
    g4 = -11.09
    g5 = 0.43664
    ...
    func_clara_mixta <- function(i_ = 5) {
      ...
      if (class(dist_D_d[[i_-1]]) == "data.frame") {
        ...
        if (edad[i_] <= 20) {
          ...
          Dg_a_ = g0+g1*(100/N_a_^g2)+g3*Ho[i_]
        }
      }
    ...
    func_trat <- function(tratam_ = "clareo", i_ = 2) {
      if (tratam_ == "") {
        ...
        if (edad[i_] <= 20) {
          ...
          Dg_d_ = g0+g1*(100/N_d_^g2)+g3*Ho[i_]
        }
        G_d_ = Dg_d_^2*pi*N_d_/40000
        if (edad[i_] <= 20) {
          V_d_ <- b3*Dg_d_^b4*Ho[i_]^b5*N_d_^b6
        } else {
          V_d_ = g4+g5*G_d_*Ho[i_]
        }
    ...
    ab_objetivo_ = c(20,25,30,40) # c(15,20,25)
    ...
    peso_G_ = c(0.2, 0.3) #c(0.20, 0.225, 0.25)
