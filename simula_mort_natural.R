...
dat_ordenaciones <- masas_Fs_vol_no_nulo_municipio %>%
  select(Edad, Area_Basimetrica, Altura_Dominante, Volumen_Total, Superficie_.ha.) %>%
  ...
  select(Edad_media, AB, Vol_medio) %>%
  rename(Edad = Edad_media, G_a_ = AB, V_a_ = Vol_medio)

ggplot(dat_ordenaciones %>% filter(Volumen_Total>0 & Sup_ha > 1), aes(x=Edad_media, y = Volumen_Total/Sup_ha))+geom_point()
ggplot(dat_ordenaciones %>% filter(Volumen_Total>0 & Sup_ha > 1), aes(x=Edad_media, y = AB))+geom_point()
