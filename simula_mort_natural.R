#Simula los escenarios de evolución bajo mortalidad natural

grupo_ <- "mortalidad_natural"

densidades_iniciales <- c(7000,5000)
rotaciones <- c(200, 190, 140, 120, 180, 160, 115) # las rotaciones de los escenarios H de GOFagus
rotaciones <- c(rotaciones, 2*rotaciones) #se simula también al doble de la rotación
rotaciones <- unique(rotaciones)

indices_cal <- c(25,22,19,16,13)

lista_ <- expand.grid(IS_ = indices_cal, densidades_iniciales = densidades_iniciales, rotaciones = rotaciones, stringsAsFactors = FALSE) %>%
  rowwise() %>%
  mutate(lista_var = list(c(IS_,densidades_iniciales,rotaciones)))

funcion_clases_mortalidad <- function(IS_ = 25, N_ini_ = 3000, edad_fin_ = 250) {
  grupo <- grupo_
  # Escenario
...
res_esc_mort <- map_dfr(lista_$lista_var, ~ funcion_clases_mortalidad(IS_ = .[1], N_ini_ = .[2], edad_fin_ = .[3]))

write.xlsx(res_esc_mort, paste0("resultados/simulaciones/",grupo_,"/","mortalidad_IFN.xlsx"))
