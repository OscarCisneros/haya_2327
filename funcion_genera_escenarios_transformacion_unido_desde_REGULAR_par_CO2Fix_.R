...
#El paso de regular a irregular se hace desde el data frame "res", para evitar el salto incongruente al pasar de reg a irreg
...
funcion_genera_escenarios_transformacion_desde_REG_unidos_ <- function( escenario = 1, grupo = grupo_transformacion,
...
print(paste0("escenario nº: ",escenario))
  ...
  resumen_variables <- data.frame(escenario.nombre = escenario.nombre,
                                  N_reg = N_reg, G_reg = G_reg, Edad_reg = Edad_reg, Tipo_masa = Tipo_masa,
  ...
  #actualizar para la simulación
    ...
    # 2º actualizar el resto de años
      ...
      #datos del monte regular hasta la edad en la que se inicia la transformación
      res_reg_ <- res_reg %>% 
        rename(Tiempo = Edad) %>% filter(Tiempo < Edad_reg) %>% 
        ...
        mutate(B_rama = B_rama7+B_rama2_7+B_rama2) %>%
        mutate(Rel_growth_foliage_ = round(B_hoja/B_fuste,3),
               Rel_growth_branches_ = round(B_rama/B_fuste,3),
               Rel_growth_roots_ = round(B_raiz/B_fuste,3)) %>%
        ...
        mutate(V_carp_ = v_carp_1+v_carp_2+v_carp_3+v_carp_4) %>%
        mutate(V_carp_ = ifelse(V_carp_ >= V_e_, V_e_, V_carp_)) %>%
      ...
      ggplot(graf_mort_ordenaciones_claras %>% filter(is.na(origen)), aes(x=Tiempo, y=V_a_, col= as.factor(IS_)))+
        ...
        geom_point(data=graf_mort_ordenaciones_claras %>% filter(origen == "claras"),
                   aes(x= Tiempo, y = V_a_, shape = trat), size = 8,  col = "red")+
        ggtitle(paste(paste0("Grupo: ",grupo),  paste0("Escenario: ",escenario.nombre), sep="\n"),
                subtitle = paste0("Línea: Evolución de Vcc m3/ha. Puntos azules: Datos de ordenaciones"))+
        theme_light()+
